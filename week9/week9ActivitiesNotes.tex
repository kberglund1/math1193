...
\begin{enumerate}
    ...
    multiplication, subtraction or division) in secret. For example: Add 2, multiply by 4, subtract 1.
    \item Player 1 writes down a function \(f\) which applies those operations,
    in order, to the variable \(x\). For example: \(f(x) = 4(x + 2) - 1\).
    \item Player 1 chooses 3 \(x\)-values in secret. Then, they plug them into their function,
    and write down the outputs. For example: \(f(0) = 7\), \(f(1) = 11\) and \(f(0.5) = 9\). So Player 1
        writes down 7, 11, 9.
    \item Player 1 shows Player 2 their function \(f\) and the outputs they wrote down.
    For example: ``My function is \(f(x) = 4(x + 2) - 1\), and my outputs are 7, 11, and 9.''
    \item Player 2 then tries to determine the original \(x\)-values that
    Player 1 chose. For example: ``Your original \(x\)-values were 0, 11, and 0.5.''
    \item Player 2 writes down a sequence of 3 operations that they
    used to determine those \(x\)-values. For example: Add 1, divide by 4, subtract 2.
    ...
    from the previous part. For example: \(g(x) = \frac{x + 1}{4} - 2\)
